	...
	private static class ClientHandler extends Thread {
		...
		public void run() {
			...
			try {
				...
				System.out.println("id: " + id + " this id " + this.id
						+ " code: " + 1 + " port: " + this.client.getPort());
				sendState(this, 1);
				...
				for (int i = 0; i < clist.size(); i++) {
					...
					if (g.id != this.id) {
						try {
							...
							/*
							 * String msg = stateMsg(2,this.id,this.state,
							 * this.xpos, this.ypos, this.zpos, this.hdg);
							 * PrintWriter out = new
							 * PrintWriter(g.client.getOutputStream(),true);
							 * out.println(msg);
							 * System.out.println("this: "+client
							 * .getPort()+":::g.client: "+g.client.getPort());
							 */
						} catch (IOException e) {
							e.printStackTrace();
						}
				...
				try {
					...
					if (client != null) {
						...
						for (Iterator<ClientHandler> i = clist.iterator(); i
								.hasNext();) {
		...
		public String stateMsg(int code, int id, String state, float xpos,
				float ypos, float zpos, float hdg) {
			// s
			String msg = "s," + Integer.toString(code) + ","
					+ Integer.toString(id) + "," + state + ","
					+ Float.toString(xpos) + "," + Float.toString(ypos) + ","
					+ Float.toString(zpos) + "," + Float.toString(hdg);
			return msg;
		...
		public void broadcastState(ArrayList<ClientHandler> clist) {
			String msg = stateMsg(3, this.id, this.state, this.xpos, this.ypos,
					this.zpos, this.hdg);
			this.broadcastMsg(clist, msg);
		}

		public void broadcastInitialState(ArrayList<ClientHandler> clist)
				throws IOException {
			int code = 0;
			System.out.printf("Client id: %d, clist size: %d\n", this.id,
					clist.size());
			...
			for (int i = 0; i < clist.size(); i++) {
				...
				if (g.id != this.id) {
					...
					System.out
							.println("id: " + g.id + " this id " + this.id
									+ " code: " + code + " port: "
									+ g.client.getPort());
					sendState(g, code);
				}
		...
		public void sendState(ClientHandler s, int code) throws IOException {
			String msg = stateMsg(code, this.id, this.state, this.xpos,
					this.ypos, this.zpos, this.hdg);
			System.out.println("sendState socket id: " + s.id + " this id: "
					+ this.id);
			PrintWriter out = new PrintWriter(s.client.getOutputStream(), true);
			out.println(msg);
		}

		public void processRpcOp(int code, int id, String state, float xpos,
				float ypos, float zpos, float hdg) {
			switch (code) {
			case 3:
				this.id = id;
